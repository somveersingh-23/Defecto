...
public class ForgotPasswordActivity extends AppCompatActivity {
    ...
    private final int otpTimeout = 60_000;
    ...
    private void sendOtpToEmail(String email) {
        ...
        new Thread(() -> {
            try {
                ...
                runOnUiThread(() -> Toast.makeText(this, "Failed to send OTP: " + ex.getMessage(), Toast.LENGTH_SHORT).show());
            }
        }).start();
    }
    ...
    private void showOtpDialog(String email) {
        ...
        btnResendOtp.setOnClickListener(v -> {
            sendOtpToEmail(email); // Resends and opens new dialog again
            otpDialog.dismiss();   // Close current dialog
        });
